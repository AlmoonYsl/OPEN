...
model = dict(
    ...
    img_roi_head=dict(
        ...
        encoder_cfg=dict(
            num_layers=1,
            layer_cfg=dict(
                attn_cfg=dict(type='DeformableFeatureAggregation3D', embed_dims=256, num_levels=2, dropout=0.1),
                ffn_cfg=dict(
                    embed_dims=256,
...
train_pipeline = [
    ...
    dict(type='PointToMultiViewDepth', downsample=16),
    ...
    dict(type='PETRFormatBundle3D', class_names=class_names,
         collect_keys=collect_keys + ['prev_exists', 'depth_map', 'depth_map_mask']),
    dict(type='Collect3D',
         keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'prev_exists',
               'depth_map', 'depth_map_mask'] + collect_keys,
         meta_keys=(
...
data = dict(
    ...
    train=dict(
        ...
        collect_keys=collect_keys + ['img', 'prev_exists', 'img_metas', 'depth_map', 'depth_map_mask'],
        queue_length=queue_length,
        test_mode=False,
        use_valid_flag=True,
        filter_empty_gt=False,
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys +
                                                                     ['img', 'img_metas'], queue_length=queue_length,
             ann_file=data_root + 'nuscenes2d_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
    test=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys +
                                                                      ['img', 'img_metas'], queue_length=queue_length,
              ann_file=data_root + 'nuscenes2d_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            # set to 0.1 always better when apply 2D pretrained.
            'img_backbone': dict(lr_mult=0.1),
        }),
    weight_decay=0.01)

optimizer_config = dict(type='Fp16OptimizerHook',
                        loss_scale='dynamic', grad_clip=dict(max_norm=35, norm_type=2))
# learning policy
lr_config = dict(
...
evaluation = dict(interval=num_iters_per_epoch *
                           num_epochs, pipeline=test_pipeline)
