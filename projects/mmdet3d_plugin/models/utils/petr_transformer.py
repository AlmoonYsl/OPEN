    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        if attn_mask is not None and attn_mask.dim() == 3:
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1).reshape(-1, len(query), len(key))
...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(self, memory, tgt, query_pos, pos_embed, attn_masks, temp_memory=None, temp_pos=None, mask=None, reg_branch=None):
        ...
        out_dec = self.decoder(
            ...
            attn_masks=attn_masks if isinstance(attn_masks, list) else [attn_masks, None],
