...
@ATTENTION.register_module()
class DeformableFeatureAggregationPE(BaseModule):
    def __init__(
            self,
            embed_dims=256,
            num_groups=8,
        ...
        super(DeformableFeatureAggregationPE, self).__init__()
        self.embed_dims = embed_dims
        self.num_groups = num_groups
        self.group_dims = (self.embed_dims // self.num_groups)
        self.num_levels = num_levels
        self.num_cams = num_cams
        self.attn = nn.MultiheadAttention(embed_dim=embed_dims, num_heads=num_groups, dropout=dropout,
    ...
    def forward(self, instance_feature, query_pos, feat_flatten, reference_points, spatial_flatten,
                level_start_index, pc_range, lidar2img_mat, img_metas):
        bs, num_anchor = reference_points.shape[:2]
        reference_points = get_global_pos(reference_points, pc_range)
        key_points = reference_points.unsqueeze(-2) + self.learnable_fc(instance_feature).reshape(bs, num_anchor,
                                                                                                  -1, 3)

        # weights = self._get_weights(instance_feature, query_pos, lidar2img_mat)

        features = self.feature_sampling(feat_flatten, spatial_flatten, level_start_index, key_points,
                                         lidar2img_mat, img_metas)
        # query = self.q_proj(instance_feature)
        query = instance_feature.repeat([self.num_cams, 1, 1]).unsqueeze(-2)
        query = query.flatten(0, 1)
        bc, ln, lvl, num_pts, _ = features.shape
        features = features.permute(0, 1, 2, 3, 4).reshape(bc, ln, -1, query.shape[-1])
        key = features.flatten(0, 1)
        out = self.attn(query=query, key=key, value=key)[0]
        out = out.reshape(bs, self.num_cams, num_anchor, -1).sum(dim=1)
        out = self.output_proj(out)
        output = out + instance_feature
        return output
    ...
    def feature_sampling(self, feat_flatten, spatial_flatten, level_start_index, key_points, lidar2img_mat,
                         img_metas):
        bs, num_anchor, num_points = key_points.shape[:3]
        ...
        for i in range(len(level_start_index)):
            if i == len(level_start_index) - 1:
                feat = feat_flatten[:, level_start_index[i]:]
            else:
                feat = feat_flatten[:, level_start_index[i]:level_start_index[i + 1]]
            h, w = spatial_flatten[i][0], spatial_flatten[i][1]
            feat = feat.reshape([bs * self.num_cams, h, w, self.num_groups, -1])
            feat = feat.permute(0, 3, 4, 1, 2).reshape([bs * self.num_cams * self.num_groups, -1, h, w])
            pos = torch.clamp(points_2d[:, :, :, i] * 2 - 1, min=-1, max=1)
            pos = pos.permute(0, 2, 1, 3, 4).reshape([bs * self.num_cams * self.num_groups, num_anchor, -1, 2])
            sampling_feat = F.grid_sample(feat, pos)
            sampling_feats.append(sampling_feat)
        sampling_feats = torch.stack(sampling_feats).permute(1, 3, 0, 4, 2)
        sampling_feats = sampling_feats.reshape(
            [bs * self.num_cams, num_anchor, len(level_start_index), num_points, -1])
