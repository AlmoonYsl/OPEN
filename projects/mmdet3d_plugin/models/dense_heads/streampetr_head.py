...
class StreamPETRHead(AnchorFreeHead):
    ...
    def forward(self, outs_roi, img_metas, topk_indexes=None, **data):
        ...
        x = data['img_feats']
        memory_center = outs_roi['location']
        B, N, C, H, W = x.shape
        num_tokens = N * H * W
        ...
        outs_dec, _, attn = self.transformer(memory, tgt, query_pos, pos_embed, attn_mask, temp_memory, temp_pos)
        ...
        mask = outputs_class.max(dim=-1).values > 0.25
        # index = outputs_class.max(dim=-1).values.max(dim=1).indices

        attn = attn[0, mask[0]].reshape([-1, 6, H, W]).sum(dim=0)
        # attn = attn[0, index].reshape([-1, 6, H, W])[0]
        scene_token = img_metas[0]['scene_token']

        import cv2
        import numpy as np
        mean = np.array([103.530, 116.280, 123.675]).reshape(3, 1, 1)
        std = np.array([57.375, 57.120, 58.395]).reshape(3, 1, 1)
        for imgid in range(N):
            cur_attn = attn[imgid]
            cur_attn = cur_attn.detach().cpu().numpy()
            cur_attn * (cur_attn > 1e-4)
            cur_attn = cur_attn * 255 / cur_attn.max()
            cur_attn = cur_attn.astype(np.uint8)
            attn_color_map = cv2.applyColorMap(
                cur_attn, cv2.COLORMAP_JET)

            ori_img = data['img'][imgid].cpu().numpy()
            img_for_show = ((ori_img * std + mean))

            img_for_show = np.transpose(
                img_for_show, (1, 2, 0)).astype(np.uint8)#[..., ::-1]

            attn_color_map = cv2.resize(
                attn_color_map, (img_for_show.shape[1], img_for_show.shape[0]), interpolation=cv2.INTER_LINEAR)

            img_with_color = img_for_show * 0.6 + 0.4 * attn_color_map
            cv2.imwrite(
                f"./debug_save/petr/attn_map_{self.counter}_{imgid}.png", img_with_color)
        self.counter += 1

        # output = '/data/jhhou/code/StreamPETR/debug_save'
        # np.save(output + './attn.npy', attn)
        # np.save(output + './query.npy', outputs_class)
